...
# getting the total size of all directories
totalDirSizeDict = {}
...
currentFreeSpace = availableSpace - totalFileSize
amountToFreeUp = spaceNeeded - currentFreeSpace

print(f'current free space: {currentFreeSpace}')
print(f'amountToFreeUp: {amountToFreeUp}')


# finding the directory size closest to the magic number without going over
closestSize = 999999999999999999999999
for thing in totalDirSizeDict:
    currentSize = totalDirSizeDict[thing]
    # print(totalDirSizeDict[thing])
    if currentSize > amountToFreeUp:
        
        if currentSize < closestSize:
